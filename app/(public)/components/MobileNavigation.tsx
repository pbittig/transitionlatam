...
export function MobileNavigation({
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <section className="absolute right-0 bottom-0 left-0 max-h-[82dvh] overflow-y-auto rounded-t-3xl bg-white px-4 pt-3 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl dark:bg-neutral-950">
            ...
            <div className="mt-4 border-t border-neutral-100 pt-4 dark:border-neutral-800">
              ...
              {isAdmin || userProfile ? (
                <form action={logout}><button className="flex min-h-12 w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-neutral-600 dark:text-neutral-300"><LogOut size={20} /> {locale === "en" ? "Sign out" : "Cerrar sesión"}</button></form>
              ) : (
                <Link href="/ingresar" onClick={() => setOpen(false)} className="flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 text-neutral-600 dark:text-neutral-300"><LogIn size={20} /> {locale === "en" ? "Sign in" : "Ingresar"}</Link>
              )}
            </div>
          </section>
        </div>
      ...
      <nav className="fixed right-0 bottom-0 left-0 z-40 grid grid-cols-4 border-t border-neutral-200 bg-white/95 px-1 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_rgba(0,0,0,.08)] backdrop-blur-xl md:hidden dark:border-neutral-800 dark:bg-neutral-950/95">
