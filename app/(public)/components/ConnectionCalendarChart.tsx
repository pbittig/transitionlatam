import type { ConnectionCalendarEntry } from "@/lib/data-access/pipeline";
...
export function ConnectionCalendarChart({ entries }: { entries: ConnectionCalendarEntry[] }) {
  if (entries.length === 0) {
    return <p className="text-sm text-neutral-500 dark:text-neutral-400">Sin proyectos con fecha estimada en este rango.</p>;
  }
  const max = Math.max(...entries.map((e) => e.capacityMw), 1);

  return (
    <div className="overflow-x-auto">
      <div className="flex min-w-[700px] items-end gap-2" style={{ height: 160 }}>
        {entries.map((e) => {
          ...
          return (
            <div key={e.yearMonth} className="flex flex-1 flex-col items-center justify-end gap-1" style={{ height: "100%" }}>
              ...
              <div
                ...
                style={{ height: `${heightPct}%`, backgroundColor: lightDark(PRINCIPAL_COLOR) }}
