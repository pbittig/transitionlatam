...
const ACTIVITY_LABELS = {
  ...
  en: { note: "Note", call: "Call", meeting: "Meeting", email: "Email", stage_change: "Stage change" },
} as const;
  ...
  return (
    <article id={`opportunity-${item.id}`} className="rounded-xl border border-neutral-200 bg-white p-3 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-50">{item.project?.name ?? item.company?.name ?? (locale === "en" ? "Unlinked opportunity" : "Oportunidad sin vínculo")}</p>
      ...
      {item.nextStep && (
        <div className={`mt-3 rounded-lg p-2.5 text-xs ${overdue ? "bg-amber-50 text-amber-900" : "bg-neutral-50 text-neutral-700"}`}>
          ...
          {item.nextStepAt && <p className="mt-1 flex items-center gap-1 opacity-75"><CalendarClock size={12} /> {new Date(`${item.nextStepAt}T12:00:00`).toLocaleDateString(locale === "en" ? "en-US" : "es-CL")}</p>}
        </div>
      )}
      {item.ownerName && <p className="mt-2 text-[11px] text-neutral-400">{locale === "en" ? "Owner" : "Responsable"}: <span className="font-medium text-neutral-600">{item.ownerName}</span></p>}

      <details className="group mt-3 border-t border-neutral-100 pt-3">
        <summary className="flex cursor-pointer list-none items-center gap-1.5 text-xs font-semibold text-brand-deep"><Pencil size={13} /> {locale === "en" ? "Edit opportunity" : "Editar oportunidad"}</summary>
        <form action={updateOpportunity} className="mt-3 grid gap-2">
          ...
          <select name="stage" defaultValue={item.stage} className="rounded-lg border border-neutral-300 bg-white px-2 py-2 text-xs">
            {OPPORTUNITY_STAGES.map((stage) => <option key={stage} value={stage}>{opportunityStageLabel(stage, locale)}</option>)}
          </select>
          <textarea name="description" defaultValue={item.description ?? ""} required rows={3} aria-label={locale === "en" ? "Commercial context" : "Contexto comercial"} className="rounded-lg border border-neutral-300 px-2 py-2 text-xs" />
          ...
          <button className="rounded-lg bg-brand-primary px-3 py-2 text-xs font-semibold text-neutral-950">{locale === "en" ? "Save changes" : "Guardar cambios"}</button>
        </form>
      </details>

      <details className="group mt-3 border-t border-neutral-100 pt-3">
        <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-semibold text-neutral-700">
          <span className="flex items-center gap-1.5"><History size={13} /> {locale === "en" ? "History" : "Historial"}</span>
        ...
        <form action={addOpportunityActivity} className="mt-3 grid gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <select name="activityType" defaultValue="note" className="rounded-lg border border-neutral-300 bg-white px-2 py-2 text-xs">
              <option value="note">{locale === "en" ? "Note" : "Nota"}</option><option value="call">{locale === "en" ? "Call" : "Llamada"}</option><option value="meeting">{locale === "en" ? "Meeting" : "Reunión"}</option><option value="email">{locale === "en" ? "Email" : "Correo"}</option>
            </select>
            <input name="occurredAt" type="date" defaultValue={today} aria-label={locale === "en" ? "Activity date" : "Fecha de actividad"} className="rounded-lg border border-neutral-300 px-2 py-2 text-xs" />
          </div>
          <textarea name="note" required rows={2} placeholder={locale === "en" ? "What happened?" : "¿Qué ocurrió?"} className="rounded-lg border border-neutral-300 px-2 py-2 text-xs" />
          <button className="rounded-lg border border-brand-primary px-3 py-2 text-xs font-semibold text-brand-deep">{locale === "en" ? "Add activity" : "Registrar actividad"}</button>
        </form>
        <ol className="mt-3 space-y-2 border-l border-neutral-200 pl-3">
          {item.activities.map((activity) => (
            <li key={activity.id} className="text-[11px] leading-4 text-neutral-500">
              <p><span className="font-semibold text-neutral-700">{ACTIVITY_LABELS[locale][activity.type]}</span> · {new Date(activity.occurredAt).toLocaleDateString(locale === "en" ? "en-US" : "es-CL")}</p>
              <p>{activity.note}</p>
              {activity.authorName && <p className="text-neutral-400">{activity.authorName}</p>}
            </li>
          ))}
          {!item.activities.length && <li className="text-[11px] text-neutral-400">{locale === "en" ? "No activities recorded yet." : "Aún no hay actividades registradas."}</li>}
        </ol>
      </details>
    </article>
  );
