      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
        <label className="relative min-w-60 flex-1">
          ...
          <input
            ...
            placeholder="Proyecto, empresa, contacto o responsable..."
        ...
        >
          <CircleAlert size={15} /> Requieren atención
        </button>
      </div>
      {opportunities.length === 0 && (
        <div className="rounded-2xl border border-dashed border-neutral-300 p-7 text-center dark:border-neutral-700">
          <p className="font-medium text-neutral-800 dark:text-neutral-100">Tu pipeline está listo para comenzar</p>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-neutral-500 dark:text-neutral-400">Registra una oportunidad arriba o selecciona “Agregar al CRM” desde un proyecto. Idealmente agrega desde el inicio un responsable y una próxima acción.</p>
        </div>
      )}
      {opportunities.length > 0 && filtered.length === 0 && (
        <p className="rounded-xl border border-dashed border-neutral-300 p-5 text-center text-sm text-neutral-500 dark:border-neutral-700 dark:text-neutral-400">No hay oportunidades que coincidan con los filtros seleccionados.</p>
      )}
                ...
                <article key={item.id} id={`opportunity-${item.id}`} className="rounded-xl border border-neutral-200 bg-white p-3 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                  ...
                  {item.nextStep && (
                    <div className={`mt-3 rounded-lg p-2.5 text-xs ${item.nextStepAt && item.nextStepAt < today ? "bg-amber-50 text-amber-900 dark:bg-amber-500/10 dark:text-amber-200" : "bg-neutral-50 text-neutral-700 dark:bg-neutral-800 dark:text-neutral-200"}`}>
                      <p className="font-medium">Próximo: {item.nextStep}</p>
                  ...
                  {item.ownerName && <p className="mt-2 text-[11px] text-neutral-400">Responsable: <span className="font-medium text-neutral-600 dark:text-neutral-300">{item.ownerName}</span></p>}
                  <form action={updateOpportunityStage} className="mt-3">
                    ...
                    <select name="stage" defaultValue={item.stage} className="w-full rounded-lg border border-neutral-300 bg-transparent px-2 py-2 text-xs dark:border-neutral-700">
                      <option value={item.stage}>Cambiar etapa…</option>
                    ...
                    <button className="mt-2 text-xs font-semibold text-brand-deep hover:underline dark:text-brand-primary">Guardar cambio</button>
                  </form>
                </article>
              ))}
            </Panel>
